...
export const SemesterAPI = {
  create: async (input: { id?: string; name: string; units: number }) =>{
    await client.models.Semester.create(
      input,
      permission,
    );
  },
  ...
  await client.models.Semester.list({
   ...
   update: async (updatedSemester: SemesterData) => {
    ...
    console.log("Original Semester fetched:", originalSemester);
    if (!originalSemester) {
        throw new Error(`Semester with ID ${updatedId} not found.`);
    }

    
    const scalarUpdates: Partial<{ name: string; units: number; owner: string }> = {};
    ...
    const coursesToAdd: CourseCreateInput[] = updatedCourses.filter(course => {
    ...
    if (hasScalarChanges) {
        console.log(`Executing scalar update for Semester ${updatedId}`);
        await client.models.Semester.update({ id: updatedId, ...scalarUpdates }, permission);
    }

    
    if (coursesToDelete.length > 0) {
        console.log(`Executing ${coursesToDelete.length} course deletions.`);
        await Promise.all(coursesToDelete.map(course => 
    ...
    if (coursesToAdd.length > 0) {
        console.log(`Executing ${coursesToAdd.length} course additions.`);
        await Promise.all(coursesToAdd.map(courseInput => 
    ...
    const coursesToUpdate = updatedCourses.filter(course => course.id && originalIdSet.has(course.id));

    if (coursesToUpdate.length > 0) {
        console.log(`Checking ${coursesToUpdate.length} existing courses for updates.`);
        const updatePromises = coursesToUpdate.map(updatedCourse => {
            const originalCourse = originalCourses.find(c => c.id === updatedCourse.id);
    ...
    console.log(`All operations complete. Fetching final state for Semester ${updatedId}`);
